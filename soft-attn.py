...
def get_args():
    ...
    parser.add_argument('--env', default='Breakout-v4', type=str, help='gym environment') #BreakoutDeterministic-v4, SeaquestDeterministic-v4
...
class NNPolicy(nn.Module): # an actor-critic neural network
    def __init__(self, channels, memsize, num_actions):
        ...
        self.Wa = nn.init.xavier_uniform_(torch.empty(self.out_channels,1))
        self.Wh = nn.init.xavier_uniform_(torch.empty(memsize,1))
        self.gru = nn.GRUCell(self.out_channels, memsize)
    ...
    def forward(self, inputs, train=True, hard=False):
        ...
        # ------- Attention unit Start -------
        attn_input = x.view(-1,self.out_channels,25)
        m_concat = [torch.matmul(attn_input[:,:,i], self.Wa) * torch.matmul(hx, self.Wh) for i in range(25)]
        m_concat = torch.cat([m_concat[i] for i in range(len(m_concat))])
        alpha = F.softmax(m_concat,dim=0)
        attn_output = torch.cat([attn_input[:,:,i]*(alpha[i,:].item()) for i in range(25)])
        attn_output = torch.sum(attn_output,dim=0)
        # ------- Attention unit End -------
        hx = self.gru(attn_output.view(-1,self.out_channels), (hx))
        return self.critic_linear(hx), self.actor_linear(hx), hx, alpha
    ...
    while info['frames'][0] <= 30e6 or args.test: # openai baselines uses 40M frames...we'll use 80M
        ...
        for step in range(args.rnn_steps):
            ...
            if num_frames % 2e6 == 0: # save every 2M frames
                ...
                torch.save(shared_model.state_dict(), args.save_dir+'model_sam_BreakoutDeterministic-v4.{:.0f}.tar'.format(num_frames/1e6))
...
def attention_heatmap(alpha):
    conv_size = int(np.sqrt(alpha.shape[0]))
    heatmap = np.reshape(alpha.detach().numpy(), (conv_size, conv_size))
    upsampled_heatmap = skimage.transform.pyramid_expand(heatmap, upscale = 32 , sigma=20)
...
def test(load_dir):
    ...
    while not done:
        i+=1
        value, logit, hx, alpha = model((state.view(1,1,80,80), hx))
        #action = torch.argmax(F.log_softmax(logit, dim=-1))
        logp = F.log_softmax(logit, dim=-1)
        action = torch.exp(logp).multinomial(num_samples=1).data[0]
        heatmap = attention_heatmap(alpha)
        state, reward, done, _ = env.step(action.numpy()[0])
